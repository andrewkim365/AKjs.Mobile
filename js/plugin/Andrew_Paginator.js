...
/*-----------------------------------------------Andrew_Paginator-------------------------------------------*/
(function ($) {
    $.Andrew_Paginator = function (el, options) {
        if(!(this instanceof $.Andrew_Paginator)){
            return new $.Andrew_Paginator(el, options);
        }
        var self = this;
        self.$container = $(el);
        self.$container.data('Andrew_Paginator', self);
        self.init = function () {
        ...
        self.verify = function () {
            ...
            if (!self.isNumber(opts.totalPages)) {
                throw new Error('[Andrew_Paginator] type error: totalPages');
            }
            if (!self.isNumber(opts.totalCounts)) {
                throw new Error('[Andrew_Paginator] type error: totalCounts');
            }
            if (!self.isNumber(opts.pageSize)) {
                throw new Error('[Andrew_Paginator] type error: pageSize');
            }
            if (!self.isNumber(opts.currentPage)) {
                throw new Error('[Andrew_Paginator] type error: currentPage');
            }
            if (!self.isNumber(opts.visiblePages)) {
                throw new Error('[Andrew_Paginator] type error: visiblePages');
            }
            if (!opts.totalPages && !opts.totalCounts) {
                throw new Error('[Andrew_Paginator] totalCounts or totalPages is required');
            }
            if (!opts.totalPages && opts.totalCounts && !opts.pageSize) {
                throw new Error('[Andrew_Paginator] pageSize is required');
            }
            if (opts.totalCounts && opts.pageSize) {
                opts.totalPages = Math.ceil(opts.totalCounts / opts.pageSize);
            }
            if (opts.currentPage < 1 || opts.currentPage > opts.totalPages) {
                throw new Error('[Andrew_Paginator] currentPage is incorrect');
            }
            if (opts.totalPages < 1) {
                throw new Error('[Andrew_Paginator] totalPages cannot be less currentPage');
            }
        };
        self.extendJquery = function () {
            $.fn.Andrew_PaginatorHTML = function (s) {
        ...
        self.callMethod = function (method, options) {
            switch (method) {
                ...
                case 'destroy':
                    ...
                    self.$container.removeData('Andrew_Paginator');
                    break;
                default :
                    throw new Error('[Andrew_Paginator] method "' + method + '" does not exist');
            }
    ...
    $.Andrew_Paginator.defaultOptions = {
        first_text: 'First',
        prev_text: 'Previous',
        next_text: 'Next',
        last_text: 'Last',
    ...
    $.fn.Andrew_Paginator = function () {
        var self = this,
            args = Array.prototype.slice.call(arguments);
        if (typeof args[0] === 'string') {
            var $instance = $(self).data('Andrew_Paginator');
            if (!$instance) {
                throw new Error('[Andrew_Paginator] the element is not instantiated');
            } else {
                return $instance.callMethod(args[0], args[1]);
            }
        } else {
            return new $.Andrew_Paginator(this, args[0]);
        }
    };
