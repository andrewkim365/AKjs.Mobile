/*! Andrew.Mobile.Plugin by jQuery MobileWebApp Script v1.0.1 Stable --- Copyright Andrew.Kim | (c) 20170808 ~ 20180329 andrew.mobile.plugin.js license */
...
function Andrew_Router(setting){
    ...
    if(option.Router== true) {
        layout = $.ajax({
            url: "layout/main.html",
            async: false,
            success:function () {
                hash = "router/"+location.hash.substring(1);
                option.success(hash);
            },
            error:function () {
                console.log("Router Ajax Load Error!");
            }
        });
        ...
        $(window).each(function () {
            if (location.hash.substring(1) != "") {
                htmlobj = $.ajax({
                    url: "router/"+location.hash.substring(1),
                    async: false,
                    success:function () {
                        hash = "router/"+location.hash.substring(1);
                        option.success(hash);
